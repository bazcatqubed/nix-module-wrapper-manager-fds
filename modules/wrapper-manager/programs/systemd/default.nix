    ...
    config = {
      ...
      environment = lib.mkMerge [
        ...
        (lib.mkIf cfg.enableCommonPathFromEnvironment {
          PATH = lib.mkIf cfg.enableCommonPathFromEnvironment (
...
in
{
  ...
  options.programs.systemd = {
    ...
    enableCommonPathFromEnvironment = lib.mkEnableOption "include paths from {option}`environment.pathAdd`";

    enableInheritEnvironmentVariables = lib.mkEnableOption "include environment variables from {option}`environment.variables`";

    system =
      let
        mkSystemUnitDefinition =
          unitTypeSingular:
          lib.mkOption {
      ...
      in
      {
        units = lib.mkOption {
          ...
          '';
          default = { };
          type = with lib.types; attrsOf (submodule (systemdUtils.submodules.units ++ [ sharedUnitConfig ]));
        };
        ...
        timers = mkSystemUnitDefinition "timer" // {
          type = with lib.types; attrsOf (submodule (systemdUtils.submodules.timers ++ [ sharedUnitConfig ]));
          example = lib.literalExpression ''
            {
              hello = {
        ...
        targets = mkSystemUnitDefinition "target" // {
          type =
            with lib.types;
            attrsOf (submodule (systemdUtils.submodules.targets ++ [ sharedUnitConfig ]));
        };
        ...
        mounts = mkSystemUnitDefinition "mount" // {
          type = with lib.types; attrsOf (submodule (systemdUtils.submodules.mounts ++ [ sharedUnitConfig ]));
        };

        automounts = mkSystemUnitDefinition "automount" // {
          type =
            with lib.types;
            attrsOf (submodule (systemdUtils.submodules.automounts ++ [ sharedUnitConfig ]));
        };

        slices = mkSystemUnitDefinition "slice" // {
          type = with lib.types; attrsOf (submodule (systemdUtils.submodules.slices ++ [ sharedUnitConfig ]));
        };

        paths = mkSystemUnitDefinition "path" // {
          type = with lib.types; attrsOf (submodule (systemdUtils.submodules.paths ++ [ sharedUnitConfig ]));
        };
      };
  ...
  config =
    let
      ...
      addTimerUnitByServicesWithStartAt =
        services:
        addUnitWithCondition services (_: v: v.startAt != [ ]) (
          _: v: {
            wantedBy = [ "timers.target" ];
      ...
      addSocketUnitByServicesWithListenOn =
        services:
        addUnitWithCondition services (_: v: v.listenOn != [ ]) (
          _: v: {
      ...
      addPathUnitByServicesWithWatchFilesFrom =
        services:
        addUnitWithCondition services (_: v: v.watchFilesFrom != [ ]) (
          _: v: {
    ...
    in
    {
      ...
      files = lib.mkMerge [
        (lib.mkIf (cfg.system.units != { }) {
          "/etc/systemd/system".source = wrapperManagerLib.systemd.generateUnits {
            inherit (cfg.system) units;
          };
        })
