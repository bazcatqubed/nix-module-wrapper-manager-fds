...
let
  ...
  mkSystemdOption =
    name: unitTypeSingular: systemdUnitAttr:
    let
    in
    {
...
in
{
  ...
  config =
    let
      wrappersWithSystemdEnabled =
        variant: lib.filterAttrs (_: v: v.systemd.enable && v.systemd.variant == variant) config.wrappers;

      mkSystemdUnitConfig =
        name: wrapperConfig:
        let
          inherit (wrapperConfig.systemd)
            serviceUnit
            timerUnit
            pathUnit
            socketUnit
    ...
    in
    {
      programs.systemd.user.units = lib.concatMapAttrs mkSystemdUnitConfig (
        wrappersWithSystemdEnabled "user"
      );

      programs.systemd.system.units = lib.concatMapAttrs mkSystemdUnitConfig (
        wrappersWithSystemdEnabled "system"
      );
    };
}
