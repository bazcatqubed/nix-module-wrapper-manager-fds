...
* Unlike the original version, this is primarily used for creating wrappers for shadowing an executable. footnote:[Though, this project does facilitate easily creating a package for desktop usage (e.g., XDG desktop entries).]


[#why-not-use-wrapper-manager]
=== What wrapper-manager won't be

While wrapper-manager uses the same module system as NixOS and home-manager, there are some boundaries this project won't cross.

* The ecosystem wrapper-manager builds will not facilitate building larger-than-typical-packages, specifically https://nix.dev/manual/nix/2.25/package-management/profiles[Nix profiles] which is what NixOS and home-manager builds. footnote:[Even if one could make the argument that NixOS and home-manager outputs can be treated the same as any other packages, it is easy to "scope creep" this into oblivion.]

* There won't be any focus on application configuration especially on desktop ecosystem where it typically relies on other programs, even their configuration (e.g., GNOME apps relying on GSettings [and dconf], KDE applications persisting on locations with cascading configuration system).
This includes something like a configurative design system such as https://github.com/nix-community/stylix[stylix] or https://github.com/tinted-theming/[Tinted Theming].

* Service management is out of the question so there's no `services` namespace similarly found in NixOS and home-manager. footnote:[We could support modular services though if there's suitable design for them [e.g., generating simple scripts for them].]
...
[source, nix]
----
include::example$configs/data-format-files/default.nix[tag=!*]
----
...
wrapper-manager-fds has support for https://nixos.org/manual/nixos/stable/#modular-services[Modular services] which allows easy instantiation of services through different configuration frameworks (e.g., NixOS, process-manager).
Though, just like the feature itself, it is experimental (as of NixOS 25.11) so it is not imported by default.
You have to manually import the module yourself to make this work like in the following code block.

[source, nix]
----
{
  # If you're using inside of the wrapper-manager configuration, you can use
  # the `modulesPath` module argument which is set through `specialArgs`.
  imports = [ "${modulesPath}/modular-services" ];

  # If you're using it through the user entrypoint, they're available from
  # `wrapperModules` attribute.
  imports = [ wmEntrypoint.wrapperModules.modular-services ];
}
----

[WARNING]
====
All of the features and options in this section are experimental and can change without any warning.
====

Once you have imported them, you can easily create wrappers configured through the modular services through xref:./wm-options.adoc#_environment_services[`environment.services`].

[source, nix]
----
include::example$configs/modular-services.nix[tag=!*]
----

There is also an integration module with systemd though that requires importing yet another module for it (i.e., `"${modulesPath}/programs/systemd/modular-services.nix"` if inside the configuration, `wrapperModules.systemd-modular-services-integration`).
Though once it is imported, you don't have to touch anything else as it will generate them systemd units in the output.
