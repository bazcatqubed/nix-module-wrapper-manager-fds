...
in
{
  options.custom-systemd.units = lib.mkOption {
    ...
    type =
      with lib.types;
      attrsOf (submodule (wrapperManagerLib.systemd.submodules.units ++ [ customModule ]));
  };
  ...
  options.custom-systemd.services = lib.mkOption {
    ...
    type =
      with lib.types;
      attrsOf (submodule (wrapperManagerLib.systemd.submodules.services ++ [ customModule ]));
  };
  ...
  options.custom-systemd.timers = lib.mkOption {
    ...
    type =
      with lib.types;
      attrsOf (submodule (wrapperManagerLib.systemd.submodules.timers ++ [ customModule ]));
  };
  ...
  options.custom-systemd.paths = lib.mkOption {
    ...
    type =
      with lib.types;
      attrsOf (submodule (wrapperManagerLib.systemd.submodules.paths ++ [ customModule ]));
  };
  ...
  options.custom-systemd.sockets = lib.mkOption {
    ...
    type =
      with lib.types;
      attrsOf (submodule (wrapperManagerLib.systemd.submodules.sockets ++ [ customModule ]));
  };
  ...
  options.custom-systemd.slices = lib.mkOption {
    ...
    type =
      with lib.types;
      attrsOf (submodule (wrapperManagerLib.systemd.submodules.slices ++ [ customModule ]));
  };
  ...
  config =
    ...
    lib.mkMerge [
      ...
      # --- IMPLEMENTATION ---
      {
        ...
        files."${customSystemdDir}".source = wrapperManagerLib.systemd.generateUnits {
          inherit (cfg) units;
        };
